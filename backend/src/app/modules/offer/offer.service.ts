...
class OfferService {
  ...
  async getOffersFromDB(filterPayload: OffersFilterPayload, paginationOptions: IPaginationOptions) {
    ...
    if (!filterPayload.status) {
      whereConditions['status'] = { $ne: OfferStatus.DELETED };
    }
  ...
  async getEndingSoonOffersFromDB(paginationOptions: IPaginationOptions) {
    ...
    // Current date and 7 days later
    const now = new Date();
    const sevenDaysLater = new Date();
    sevenDaysLater.setDate(now.getDate() + 30);

    // Find offers that are running and ending in the next 7 days
    const offers = await OfferModel.find({
      status: OfferStatus.Running,
      endDate: { $gte: now, $lte: sevenDaysLater }, // ending within 7 days
    })
      .sort({ [sortBy]: sortOrder })
      .skip(skip)
      .limit(limit);
    ...
    return {
      ...
      meta: {
        ...
        limit,
        totalResults,
        total: totalResults, // total pages for this filtered query
      },
    };
  }
  ...
  async updateExpiredDeadlineOffers() {
    ...
    await OfferModel.updateMany(
      {
        ...
        status: {
          $in: [OfferStatus.Running, OfferStatus.PAUSED],
        },
      },
      {
        endDate: newEndDate,
