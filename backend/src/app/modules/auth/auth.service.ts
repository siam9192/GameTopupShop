...
class AuthService {
  ...
  async changePassword(authUser: AuthUser, payload: ChangePasswordPayload) {
    ...
    if (authUser.role === UserRole.CUSTOMER) {
      ...
      // Find the user by ID and include the password field
      const admin = await AdministratorModel.findById(authUser.userId, { password: true });
      // Check if user exists
      if (!admin) {
        throw new AppError(httpStatus.BAD_REQUEST, 'User not found.');
      }
    ...
    if (authUser.role === UserRole.CUSTOMER) {
      ...
      // Update the password
      const updateResult = await AdministratorModel.updateOne(
        { _id: objectId(authUser.userId) },
        { password: newHashedPassword }
      );
  ...
  async callback(payload: CallbackPayload) {
    ...
    if (payload.provider === 'google') {
      ...
      // 1. Fetch profile from Facebook Graph API
      const res = await fetch(
        `https://graph.facebook.com/me?fields=id,name,first_name,last_name,email,picture.width(400).height(400)&access_token=${payload.access_token}`
      );
      ...
      // 3. Create if new customer
      if (!customer) {
        customer = await customerService.createCustomer({
          facebookId: profile.id,
          name: {
            first: profile.first_name ?? profile.name?.split(' ')[0] ?? '',
            last: profile.last_name ?? profile.name?.split(' ')[1] ?? '',
          },

          ...(profile.picture ? { profilePicture: profile.picture.data.url } : {}),
