...
router.patch(
  '/:id/cancel',
  auth(UserRole.CUSTOMER),
  walletSubmissionController.cancelWalletSubmission
);
...
router.get(
  '/my/recent',
  auth(UserRole.CUSTOMER),
  walletSubmissionController.getMyRecentWalletSubmissions
);
router.get('/my/:id', auth(UserRole.CUSTOMER), walletSubmissionController.getWalletSubmissionById);
