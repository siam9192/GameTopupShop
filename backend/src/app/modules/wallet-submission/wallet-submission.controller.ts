...
class WalletSubmissionController {
  ...
  cancelWalletSubmission = catchAsync(async (req, res) => {
    const result = await walletSubmissionService.cancelWalletSubmissionIntoDB(req.params.id);
    sendSuccessResponse(res, {
      statusCode: httpStatus.OK,
      data: result,
      message: 'Wallet submission canceled successfully',
    });
  });
  ...
  getMyRecentWalletSubmissions = catchAsync(async (req, res) => {
    const result = await walletSubmissionService.getMyRecentSubmissionsFromDB(
      Pick(req.query, ['days']),
      paginationOptionPicker(req.query)
    );
    sendSuccessResponse(res, {
