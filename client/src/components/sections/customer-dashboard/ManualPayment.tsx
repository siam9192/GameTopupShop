import ManualPaymentMethodCard from '@/components/cards/ManualPaymentMethodCard';
import ManualPaymentForm from '@/components/forms/ManualPaymentAddMoneySubmissionForm';
import { getPublicManualPaymentMethodsQuery } from '@/query/services/manual-payment-method';
import {
  Box,
  Button,
  Chip,
...
function ManualPayment() {
  ...
  const { data, isLoading } = getPublicManualPaymentMethodsQuery([
    {
      name: 'page',
      value: page,
    },
            ...
            >
              {methods.map(method => (
                <Grid size={1} key={method._id || method.name}>
                  <ManualPaymentMethodCard method={method} />
                </Grid>
              ))}
            </Grid>
